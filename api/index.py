...
def execute_sql(sql: str, params: list = None) -> dict:
    ...
    response = requests.post(TURSO_URL, headers=headers, json=data, timeout=30)
...
def check_revocation(card_number: str) -> bool:
    result = execute_sql("SELECT COUNT(*) FROM card_revocation WHERE card_number = ?", [card_number])
    try:
...
def get_card_info(card_number: str) -> Optional[Dict]:
    ...
    try:
        if result and len(result) > 0 and 'results' in result[0]:
            rows = result[0]['results'].get('rows', [])
            columns = result[0]['results'].get('columns', [])
            if rows and len(rows) > 0:
...
def activate_card(card_number: str, machine_fingerprint: str) -> Tuple[bool, str, Dict]:
    ...
    execute_sql("UPDATE cards SET status = '已激活', machine_fingerprint = ?, activate_time = ?, expire_time = ? WHERE card_number = ?", 
